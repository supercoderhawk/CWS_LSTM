...
class CWS(object):
    ...
    def get_best(self, sentence, ans, flag):
        ...
        for i in xrange(sen_len):
            ...
            y_ans_in = ans
            if (i > 0):
                cost += output[int(y_pred_in[i])] + self.params['A'][y_pred_in[i - 1]][y_pred_in[i]] - output[
                    int(y_ans_in[i])] - self.params['A'][y_ans_in[i - 1]][y_ans_in[i]]
            else:
                cost += output[int(y_pred_in[i])] + self.params['A'][0][y_pred_in[i]] - output[int(y_ans_in[i])] - \
                        self.params['A'][0][y_ans_in[i]]
        # print cost
        return (ret, cost)
    ...
    def fit(self):
        ...
        for epoch in xrange(self.n_epochs):
            (self.data.data_train, self.data.label_train) = self.data.shuffle(self.data.data_train,
                                                                              self.data.label_train)
            ae_costs = 0.0
